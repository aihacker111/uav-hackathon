...
Public entry points consumed by jde.py:
  build_aerial_mot_transforms() -> Compose
      Returns the full pre-letterbox PIL augmentation pipeline.
      Call once in dataset __init__; apply per-sample in get_data.
"""
import io
import random
...
def build_aerial_mot_transforms() -> Compose:
    """
    PIL-based pre-letterbox augmentation pipeline for aerial MOT at 892×512.

    Target resolution: 892 W × 512 H  (aspect ≈ 1.74, both divisible by 64).

    Replaces the old apply_weather_light_color + bias_crop + random_affine
    calls in jde.py.  Instantiate once in the dataset __init__, then call
    per sample in get_data (before letterbox).
    ...
    Key design decisions:
      ...
      • RandomHorizontalFlip replaces the standalone lr_flip in get_data.
      • Post-letterbox random_affine in get_data MUST be skipped when this
        pipeline is active (geometric augmentation is handled here instead).

    Input / output: (PIL.Image.Image [RGB], target_dict) where target_dict
    contains at minimum {"boxes": FloatTensor[N,4] xyxy, "labels": LongTensor[N],
    "ids": LongTensor[N]}.
    """
    return Compose([
        # ── geometric ──────────────────────────────────────────────────────────
